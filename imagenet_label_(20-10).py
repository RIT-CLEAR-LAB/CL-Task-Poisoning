...
def main(args=None, log_filename=None, device="cuda:2"):
...
for buffer_size, model, label_flip_percentage in itertools.product(buffer_sizes, models, label_flip_percentages):
    args_dict = dict(
        dataset='seq-tinyimg-label-poisoning',
        model=model,
        n_epochs=50,
        nowand=1,
        non_verbose=1,
    ...
    optim_params = best_args['seq-tinyimg'][model][bs_config]
    args_dict.update(optim_params)
    args = argparse.Namespace(**args_dict)
    file = f"results/label_poison/imagenet_{model}_{buffer_size}_{label_flip_percentage}_(20-10).json"
    main(args, file)